 ...
 * Provides CRUD operations and custom queries for artist data access.
 */
@Repository
public interface ArtistRepository extends JpaRepository<Artist, Long> 
{
    ...
    /**
     * Custom query to search artists by name containing a keyword (case-insensitive).
     *
     * @param keyword the keyword to search for in artist names
     ...
     */
    @Query("SELECT DISTINCT al.artist FROM Album al " +
           "WHERE al.stockQuantity > 0")
    List<Artist> findArtistsWithAvailableStock();

    /**
     * Custom query to find artists with their album count.
     * Returns artists ordered by album count descending.
     *
     * @return list of artists with at least one album, ordered by album count
     */
    @Query("SELECT al.artist FROM Album al " +
           "GROUP BY al.artist " +
           "HAVING COUNT(al) > 0 " +
